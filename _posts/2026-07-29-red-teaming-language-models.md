...
OpenAI's closing step is what makes a program compound rather than repeat: convert human findings into automated evaluations. Their DALL·E 3 work is the worked example, where human findings seeded a synthetic set that trained a prompt-rewrite classifier. A red team whose output is a PDF tells you where you stood last quarter; a regression suite makes those findings impossible to reintroduce, so each campaign starts from the last one's floor.
...
For structuring what humans do, the grounded-theory study of in-the-wild red teamers is the best inventory available, giving 12 strategies and 35 techniques across five families ([Inie et al. 2023](https://arxiv.org/abs/2311.06237)):
...
The corollary reorders the standard process: **red teaming your aligned checkpoint characterizes a configuration no adversary will use.** The costs of stripping alignment are consistently trivial, across four independent results. Fine-tuning GPT-4 on 340 examples removed its RLHF protections with up to 95% success, for under $245 ([Zhan et al. 2023](https://arxiv.org/abs/2311.05553)). A LoRA fine-tune brought Llama 2-Chat 70B to about 1% refusal on two benchmarks, for under $200 on a single GPU ([Lermen et al. 2023](https://arxiv.org/abs/2310.20624)). Between 10 and 100 adversarial examples undoes safety alignment, and fine-tuning on *benign* instruction datasets degrades it too ([Qi et al. 2024](https://arxiv.org/abs/2310.03693)). And 100 samples took ChatGPT to an 88% attack success rate, and open models past 91% ([Bhardwaj & Poria 2023](https://arxiv.org/abs/2310.14303)). Any red-team result against the aligned checkpoint has a shelf life of roughly one afternoon of attacker GPU time.
...
Filtered 6.9B models stay tamper-resistant up to 10,000 steps of adversarial fine-tuning on 305M tokens, over an order of magnitude better than existing post-training baselines, principally TAR ([Tamirisa et al. 2024](https://arxiv.org/abs/2408.00761)). The filtering pipeline costs under 1% of total training FLOPs, with no observed degradation to unrelated capabilities. That combination is not available anywhere else in the literature.
...
I would not oversell it, and neither do the authors. It is preliminary, has not been applied to production models at Anthropic, and the paper is explicit that it is unclear whether entangled capabilities make this separation feasible in the limit, whether it scales to frontier models, and even how it differs conceptually from LoRA. My own reservation is narrower. Deleting a module before release lands you back in the [previous section's](#moving-the-intervention-into-pretraining) situation, where the question is whether an adversary can retrain the capability into the weights you did ship. Selective Gradient Masking's seven-times number is the only real evidence on that, and seven times cheap is still cheap. Modular pretraining makes it easy to produce many variants; it does not make any one variant hard to tamper with. Those are different problems and the literature is much further along on the first.
...
4. **Treat the release decision as the top-level control and document it:** the decision, the criteria, the dissent, and what would have changed it. It is the only irreversible step in the pipeline. Anthropic flagged back in 2022 that their release decision was made in isolation with no community norms to appeal to ([Ganguli et al. 2022](https://arxiv.org/abs/2209.07858)), and that remains largely true.
...
**Agentic and environment-level red teaming has no published methodology.** Scale AI calls for sandbox and environment investment comparable to capability research: modeling world state (saving a password is safe on a personal machine, not on a shared one), classifying harm at the trajectory level rather than the response level, monitoring the user rather than only the output, and red-teaming the monitor itself. Nobody has published a worked methodology, and agents are already the empirical worst case, so this is the largest process-level hole in the field.
